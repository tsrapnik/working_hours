...
var $author$project$Main$workWeek = _List_fromArray(
	[
		{dayName: $author$project$Main$Monday, tasks: _List_Nil},
		{dayName: $author$project$Main$Tuesday, tasks: _List_Nil},
		{dayName: $author$project$Main$Wednesday, tasks: _List_Nil},
		{dayName: $author$project$Main$Thursday, tasks: _List_Nil},
		{dayName: $author$project$Main$Friday, tasks: _List_Nil}
	]);
var $author$project$Main$init = $author$project$Main$workWeek;
...
var $author$project$Main$stringToMinutes = function (string) {
	return $elm$core$Maybe$Just(671);
};
var $author$project$Main$update = F2(
	function (msg, model) {
		var applyToDayWithSameName = F2(
			function (_function, dayName) {
				var applyIfDayWithSameName = function (day) {
					return _Utils_eq(day.dayName, dayName) ? _function(day) : day;
				};
				return A2($elm$core$List$map, applyIfDayWithSameName, model);
			});
		var applyToTaskWithSameTaskId = F2(
			function (_function, taskId) {
				var applyToTaskWithSameId = function (day) {
					var applyIfTaskWithSameId = function (task) {
						return _Utils_eq(task.taskId.task, taskId.task) ? _function(task) : task;
					};
					return _Utils_update(
						day,
						{
							tasks: A2($elm$core$List$map, applyIfTaskWithSameId, day.tasks)
						});
				};
				return A2(applyToDayWithSameName, applyToTaskWithSameId, taskId.day);
			});
		switch (msg.$) {
			case 'AddTask':
				var dayName = msg.a;
					...
					function (day, newId) {
						return _Utils_update(
							day,
							{
								tasks: A2(
									$elm$core$List$cons,
									A2($author$project$Main$emptyTask, day.dayName, newId),
									day.tasks)
							});
					});
				var addTaskWithCorrectId = function (day) {
					var _v1 = $elm$core$List$head(day.tasks);
					if (_v1.$ === 'Just') {
						var previousTask = _v1.a;
						return A2(addTask, day, previousTask.taskId.task + 1);
					} else {
						return A2(addTask, day, 0);
					}
				};
				return A2(applyToDayWithSameName, addTaskWithCorrectId, dayName);
			case 'RemoveTask':
				var taskId = msg.a;
				return A2(applyToTaskWithSameTaskId, $elm$core$Basics$identity, taskId);
			case 'SetComment':
				var taskId = msg.a;
				var comment = msg.b;
				var setComment = function (task) {
					return _Utils_update(
						task,
						{comment: comment});
				};
				return A2(applyToTaskWithSameTaskId, setComment, taskId);
			case 'SetStartTime':
				var taskId = msg.a;
				var startTime = msg.b;
				var setComment = function (task) {
					return _Utils_update(
						task,
						{
							startTime: $author$project$Main$stringToMinutes(startTime)
						});
				};
				return A2(applyToTaskWithSameTaskId, setComment, taskId);
			default:
				...
				var setComment = function (task) {
					return _Utils_update(
						task,
						{
							stopTime: $author$project$Main$stringToMinutes(stopTime)
						});
				};
				return A2(applyToTaskWithSameTaskId, setComment, taskId);
		}
...
var $author$project$Main$daynameToString = function (dayName) {
	switch (dayName.$) {
		case 'Monday':
			return 'monday';
		case 'Tuesday':
...
var $author$project$Main$SetComment = F2(
	function (a, b) {
		return {$: 'SetComment', a: a, b: b};
	});
var $author$project$Main$SetStartTime = F2(
	function (a, b) {
		return {$: 'SetStartTime', a: a, b: b};
	});
var $author$project$Main$SetStopTime = F2(
	function (a, b) {
		return {$: 'SetStopTime', a: a, b: b};
	});
var $elm$html$Html$input = _VirtualDom_node('input');
var $elm$core$Basics$abs = function (n) {
	return (n < 0) ? (-n) : n;
};
var $author$project$Main$minutesToString = function (timeInMinutes) {
	if (timeInMinutes.$ === 'Nothing') {
		return '--:--';
	} else {
		var minutes = timeInMinutes.a;
		var maxMinutes = (99 * 60) + 59;
		var absoluteMinutes = $elm$core$Basics$abs(minutes);
		if (_Utils_cmp(absoluteMinutes, maxMinutes) < 1) {
			var remainingMinutes = absoluteMinutes % 60;
			var hours = (absoluteMinutes / 60) | 0;
			return $elm$core$String$fromInt(hours) + (':' + $elm$core$String$fromInt(remainingMinutes));
		} else {
			return '--:--';
		}
	}
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('top_row')
					]),
				_List_fromArray(
					[
						A2(
						...
						_List_fromArray(
							[
								...
								$elm$html$Html$Attributes$value(task.comment),
								$elm$html$Html$Events$onInput(
								$author$project$Main$SetComment(task.taskId))
							]),
						_List_Nil),
						A2(
						$elm$html$Html$button,
						_List_fromArray(
							[
								...
								$author$project$Main$RemoveTask(task.taskId))
							]),
						_List_fromArray(
							[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('bottom_row')
					]),
				_List_fromArray(
					[
						A2(
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$class('start_time'),
								$elm$html$Html$Attributes$type_('time'),
								$elm$html$Html$Attributes$value(
								$author$project$Main$minutesToString(task.startTime)),
								$elm$html$Html$Events$onInput(
								$author$project$Main$SetStartTime(task.taskId))
							]),
						_List_Nil),
						A2(
						$elm$html$Html$input,
						_List_fromArray(
							[
								$elm$html$Html$Attributes$class('stop_time'),
								$elm$html$Html$Attributes$type_('time'),
								$elm$html$Html$Attributes$value(
								$author$project$Main$minutesToString(task.stopTime)),
								$elm$html$Html$Events$onInput(
								$author$project$Main$SetStopTime(task.taskId))
							]),
						_List_Nil)
					]))
			]));
...
var $author$project$Main$viewDay = function (day) {
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$text(
				$author$project$Main$daynameToString(day.dayName)),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
				...
				A2($elm$core$List$map, $author$project$Main$viewTask, day.tasks)),
				A2(
				$elm$html$Html$button,
				_List_fromArray(
					[
						$elm$html$Html$Events$onClick(
						$author$project$Main$AddTask(day.dayName))
					]),
				_List_fromArray(
					[
...
var $author$project$Main$view = function (model) {
	return A2(
		...
		A2($elm$core$List$map, $author$project$Main$viewDay, model));
};
var $author$project$Main$main = $elm$browser$Browser$sandbox(
	{init: $author$project$Main$init, update: $author$project$Main$update, view: $author$project$Main$view});
_Platform_export({'Main':{'init':$author$project$Main$main(
	$elm$json$Json$Decode$succeed(_Utils_Tuple0))({"versions":{"elm":"0.19.1"},"types":{"message":"Main.Msg","aliases":{"Main.Id":{"args":[],"type":"Basics.Int"},"Main.TaskId":{"args":[],"type":"{ day : Main.DayName, task : Main.Id }"}},"unions":{"Main.Msg":{"args":[],"tags":{"AddTask":["Main.DayName"],"RemoveTask":["Main.TaskId"],"SetComment":["Main.TaskId","String.String"],"SetStartTime":["Main.TaskId","String.String"],"SetStopTime":["Main.TaskId","String.String"]}},"Main.DayName":{"args":[],"tags":{"Monday":[],"Tuesday":[],"Wednesday":[],"Thursday":[],"Friday":[]}},"Basics.Int":{"args":[],"tags":{"Int":[]}},"String.String":{"args":[],"tags":{"String":[]}}}}})}});}(this));